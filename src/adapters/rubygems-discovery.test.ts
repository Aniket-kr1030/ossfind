...
describe("RubyGemsDiscoverer", () => {
  ...
  it("rejects when every probe fails on 429, 500, or a thrown error", async () => {
    ...
    await expect(new RubyGemsDiscoverer(rateLimited).discover("query-429")).rejects.toThrow(/failed for every probe/);
    ...
    await expect(new RubyGemsDiscoverer(serverError).discover("query-500")).rejects.toThrow(/failed for every probe/);

    const throwsError: HttpClient = async () => {
      throw new Error("Network error");
    };
    await expect(new RubyGemsDiscoverer(throwsError).discover("query-throws")).rejects.toThrow(/failed for every probe/);
  });
